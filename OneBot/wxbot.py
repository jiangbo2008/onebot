#!/usr/bin/env python
# coding: utf-8
# coming from: https://raw.githubusercontent.com/liuwons/wxBot/master/wxbot.py
# will add to contribution and handle licences things
...
class WXBot:
    ...
    def __init__(self):
        ...
        self.batch_count = 50    #һ????ȡ50????ϵ?˵???Ϣ
        self.full_user_name_list = []    #ֱ?ӻ?ȡ????ͨѶ¼ʱ????ȡ??username?б?
        self.wxid_list = []   #??ȡ????wxid???б?
        self.cursor = 0   #??ȡ??ϵ????Ϣ???α?
        self.is_big_contact = False  #ͨѶ¼???????࣬?޷?ֱ?ӻ?ȡ
        #?ļ?????Ŀ¼
        ...
        # ????Ⱥ???ĳ?Ա, {'group_id1': [member1, member2, ...], ...}
        self.group_members = {}

        # ?????˻?, {'group_member':{'id':{'type':'group_member', 'info':{}}, ...}, 'normal_member':{'id':{}, ...}}
        self.account_info = {'group_member': {}, 'normal_member': {}}

        self.contact_list = []  # ??ϵ???б?
        self.public_list = []  # ?????˺??б?
        self.group_list = []  # Ⱥ???б?
        self.special_list = []  # ?????˺??б?
        self.encry_chat_room_id_list = []  # ?洢Ⱥ?ĵ?EncryChatRoomId????ȡȺ?ڳ?Աͷ??ʱ??Ҫ?õ?
    ...
    def get_group_member_name(self, gid, uid):
        ...
        :param gid: Ⱥid
        :param uid: Ⱥ?ĳ?Աid
        :return: ??????Ϣ?????? {"display_name": "test_user", "nickname": "test", "remark_name": "for_test" }
        """
    ...
    def extract_msg_content(self, msg_type_id, msg):
        ...
        if mtype == 1:
            ...
            msg_content['data'] = {'type': app_msg_type,
                                   ...
                                   'content': msg.get('Content')  # ?еĹ??ںŻᷢһ????3 4??????һ????ͼ,????ֻurl??ֻ?ܻ?ȡ??һ??,content?????????е?????
                                   }
            if self.DEBUG:
    ...
    def handle_msg(self, r):
        ...
        for msg in r['AddMsgList']:
            ...
            if msg['MsgType'] == 51 and msg['StatusNotifyCode'] == 4:  # init message
                msg_type_id = 0
                user['name'] = 'system'
                #????ȡ??????ϵ?˵?username ?? wxid?????ǻ??յ?3????????Ϣ??ֻȡ??һ??
                ...
                # print u'       ??????Ϣ??'+msg['RecommendInfo']['Content']
                # # print u'Ticket??'+msg['RecommendInfo']['Ticket'] # Ticket???Ӻ???ʱҪ??
                # print u'       ΢?źţ?'+username #δ????΢?źŵ? ??Ѷ???Զ?????һ??΢??ID ?????޷?ͨ?????? ??????????
    ...
    def proc_msg(self):
        ...
        while True:
            ...
            try:
                ...
                if retcode == '1100':  # ??΢?ſͻ????ϵǳ?
                    break
                elif retcode == '1101':  # ???????豸?ϵ?????ҳ΢??
                    break
                elif retcode == '0':
                    if selector == '2':  # ??????Ϣ
                        r = self.sync()
    ...
    def add_groupuser_to_friend_by_uid(self,uid,VerifyContent):
        """
        ??????Ⱥ????Ա???к????ύ???Ӻ???????
        uid-Ⱥ????Ա??uid   VerifyContent-?????к?????
        ???ô˽ӿڣ????ź????Ը??????ô˽ӿڣ????ź????Ը??????ô˽ӿڣ????ź????Ը???
        """
    ...
    def invite_friend_to_group(self,uid,group_name):
        """
        ?????Ѽ??뵽Ⱥ?С???????????Ⱥ????Ҫ???ô˷?????
        ????ʱ???????ȳ???ʹ??add_friend_to_group????????????ʧ??(Ret=1)ʱ???ٳ??Ե??ô˷?????
        """
        gid = ''
        # ͨ??Ⱥ????ȡȺid,Ⱥû???浽ͨѶ¼?еĻ??޷?????Ŷ
        for group in self.group_list:
    ...
    def get_icon(self, uid, gid=None):
        ...
        ??ȡ??ϵ?˻???Ⱥ?ĳ?Աͷ??
        :param uid: ??ϵ??id
        :param gid: Ⱥid??????Ϊ??None??ȡȺ?г?Աͷ????????ΪNone????ȡ??ϵ??ͷ??
        """
        if gid is None:
    ...
    def set_remarkname(self,uid,remarkname):#??????ϵ?˵ı?ע??
